...
export function computeHubLayout(flow: Flow, config: FlowConfig): FlowLayout {
  ...
  const bankOf = (n: FlowNodeT) => !!config.nodeBankLogo?.[key(n)];
  ...
  if (leftEnd)
    nodes.push({ ...box(hubCx - SPAN, railY, NODE_W, NODE_H), id: leftEnd.id, srcId: leftEnd.srcId, label: labelOf(leftEnd), kind: leftEnd.kind, lane: leftEnd.lane, lines: wrapLabel(labelOf(leftEnd)), depth: 0, onTrunk: true, brandedClient: brandedOf(leftEnd), partnerLogo: partnerOf(leftEnd), bankLogo: bankOf(leftEnd) });
  if (rightEnd)
    nodes.push({ ...box(hubCx + SPAN, railY, NODE_W, NODE_H), id: rightEnd.id, srcId: rightEnd.srcId, label: labelOf(rightEnd), kind: rightEnd.kind, lane: rightEnd.lane, lines: wrapLabel(labelOf(rightEnd)), depth: 2, onTrunk: true, brandedClient: brandedOf(rightEnd), partnerLogo: partnerOf(rightEnd), bankLogo: bankOf(rightEnd) });
  ...
  pool.forEach((p, i) => {
    ...
    nodes.push({ ...box(cx, poolY, POOL_W, POOL_H), id: p.id, srcId: p.srcId, label: labelOf(p), kind: p.kind, lane: p.lane, lines: wrapLabel(labelOf(p)), depth: 3, onTrunk: false, brandedClient: brandedOf(p), partnerLogo: partnerOf(p), bankLogo: bankOf(p) });
  });
...
export function computeNettingLayout(flow: Flow, config: FlowConfig): FlowLayout {
  ...
  const cornerLayout = (n: FlowNodeT): NodeLayout => {
    ...
    return {
      ...
      lines: wrapLabel(labelOf(n)), depth: pays ? 0 : 2, onTrunk: true, brandedClient: brandedOf(n), partnerLogo: partnerOf(n), bankLogo: bankOf(n),
    };
  };
  const nodes: NodeLayout[] = [
  ...
  const legs: LegLayout[] = flow.legs.map((l, index) => {
    ...
    // flush at the box edge — even a 2px tuck shows through the glass
...
export function computeLayout(flow: Flow, config: FlowConfig, opts: { collapsed?: boolean } = {}): FlowLayout {
  ...
  const bankOf = (n: { id: string; srcId?: string }) =>
    !!config.nodeBankLogo?.[`${flow.id}:${n.srcId ?? n.id}`];
  ...
  if (collapsed && engine) {
    ...
    flow.nodes.forEach((n) => {
      ...
      if (!idSet.has(n.id)) srcNodes.push({ id: n.id, srcId: n.srcId, label: labelOf(n), kind: n.kind, lane: n.lane, w: NODE_W, brandedClient: brandedOf(n), partnerLogo: partnerOf(n), bankLogo: bankOf(n) });
    });
    srcLegs = [];
    flow.legs.forEach((l) => {
  ...
  const legs: LegLayout[] = srcLegs.map((leg, index) => {
    ...
    if (!straight && to.x > from.x + from.w) {
      // flush at the box edges — even a 2px tuck shows through the glass
